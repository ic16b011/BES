...
int main(int argc, const char* argv[])
{
	...
	{
		semid2 = seminit(key2, 0660, (buffer * sizeof(int)));
		if(semid2 == -1)
		{
	...
	errno = 0;
	if((shmid = shmget(key, (buffer * sizeof(int)), 0660)) == -1)
	{
		if(errno != EEXIST)
		{
	...
	while(shmptr[count] != EOF)
	{
		if(count == buffer)
			count = 0;
		P(semid);
		putchar(shmptr[count]);
		count++;
		V(semid);
		V(semid2);
	}
